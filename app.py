...
def index():
    if request.method == "POST":
        ...
        labels = [elem['label'] for elem in confidence_list if elem['confidence'] >= 0.4 ]
        label_text = ""
...
def predict_recipe(input_text):
    response = requests.post("https://rhasan97-recipe.hf.space/run/predict", json={
        "data": [input_text]
    }).json()
    data = response["data"]
    return data
